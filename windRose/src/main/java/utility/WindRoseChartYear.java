...
public class WindRoseChartYear
{
    ...
    public PolarChart createRoseYear(double CALM, double[] data0, double[] data1, double[] data2, double[] angles,
    		String stno, String stnCName, String stnEName, String startDate, String endDate, Boolean runPerMonth, int month)
    {
        ...
        String textTitle = "";
        if(runPerMonth){
        	textTitle = "風花圖  Surface Wind Roses\n" + startDate.substring(0,4) + "年" + "\n" + stnCName;
        }
        else        	
        	textTitle = startDate.substring(0,4) + "年" + "\n"
            		+ stnCName + " " + stno;
	...
	public void mergeChart(ArrayList<PolarChart> chArray, String num, String filename, boolean index){
		...
		for (int i = 0; i < group; i++) { //每一個group
		    ...
		    GetPDF(subGroup, i, filename);
		}	
	}
	
	public void GetPDF(List<PolarChart> subGroup, int count, String filename){
		...
		TextBox title_2 = m2.addText(400, 35, " 風 花 圖" + "\n" + " Wind Rose" + "\n" + stnmonth + "月"
        	...
        	m2.makeChart("tmp/tmp_surface/" + filename + stnmonth + "月" +"_" + (count+2) +".pdf");
